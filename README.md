...
cd ~/services/app1/repo && git clone <repo-url> count-app

# Dockerfile を配置（infra/app1.Dockerfile を参照）
cp ~/services/app1/repo/count-app/infra/app1.Dockerfile ~/services/app1/Dockerfile

# 環境変数を設定（~/services/.env に追記）
echo "VITE_ACCESS_TOKEN=your-token-here" >> ~/services/.env

# Nginx vhost を配置
cp ~/services/app1/repo/count-app/infra/nginx.conf ~/nginx/conf.d/app1.conf
...
```bash
cd ~/services/app1/repo/count-app && git pull
cd ~/services && docker compose build count-app --no-cache && docker compose up -d
```
